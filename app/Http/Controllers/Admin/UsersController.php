...
class UsersController extends Controller
{
    ...
    public function userEdit($id)
    {
        ...
        $data['wallets'] = Wallet::with('crypto')->where('user_id', $id)->get();
    ...
    public function userUpdate(Request $request, $id)
    {
        ...
        $user->trade_limit = $userData['trade_limit'];
    ...
    public function userBalanceUpdate(Request $request, $id)
    {
        ...
        if ($userData['balance'] == null) {
            ...
            $user = User::findOrFail($id);
            $wallet = Wallet::where('id', $request->wallet)->where('user_id', $user->id)->firstOrFail();
            $code = optional($wallet->crypto)->code;
            $trx = strRandom();
            ...
            if ($userData['add_status'] == "1") {
                ...
                BasicService::makeTransaction($user, getAmount($userData['balance']), 0, '+', $trx, 'Add Balance', $code);

                $msg = [
                    'amount' => getAmount($userData['balance']),
                    'currency' => optional($wallet->crypto)->code,
                ...
                if ($userData['balance'] > $wallet->balance) {
                ...
                BasicService::makeTransaction($user, getAmount($userData['balance']), 0, '-', $trx, $remarks = 'DEDUCTED BALANCE', $code);
    ...
    public function kycAction(Request $request, $id)
    {
        ...
        if ($request->status == '1') {
            ...
            $this->userFirebasePushNotification($user, 'KYC_APPROVED', $msg);
            ...
            $this->userFirebasePushNotification($user, 'KYC_REJECTED', $msg);
