...
@section('content')
    <div class="container-fluid">
        <div class="main row">
            <div class="col-12">
                <h5>@lang('Sell') - {{$currencyCode ? strtoupper($currencyCode):'Crypto'}}</h5>
                <div class="tab-section">
                    <div class="switcher navigator">
                        <a href="{{route('user.sellCurrencies.list','all')}}"
                           class="sell tab text-uppercase {{(last(request()->segments())=='all')?'active':''}}">@lang('All')</a>
                        @forelse($sellCurrencyLists as $item)
                            <a href="{{route('user.sellCurrencies.list',[@slug(optional($item->cryptoCurrency)->code),$item->crypto_id])}}"
                               class="sell tab text-uppercase {{(last(request()->segments())==$item->crypto_id)?'active':''}}">{{optional($item->cryptoCurrency)->code}}</a>
                        @empty
                        @endforelse
                    </div>
                </div>
                <div class="search-bar">
                    <form action="{{route('user.sellCurrencies.list')}}" method="get">
                                ...
                                <div class="d-lg-flex d-block align-items-center">
                                    ...
                                    <div class="">
                                        ...
                                        @if(optional($item->user)->total_trade > 0)
                                            <span
                                                class="text-muted font-10">@lang('Completion') {{number_format(optional($item->user)->completed_trade*100/optional($item->user)->total_trade,2)}} %</span>
                                        @else
                                            <span
                                                class="text-muted font-10">@lang('Completion') 0 %</span>
                                        @endif
                                    </div>
                                </div>
                            </a>
                        ...
                        <td data-label="@lang('Action')" class="action">
                            <a href="{{route('user.sellCurrencies.tradeRqst',$item->id)}}"
                               class="btn-custom sell-currency p-6">
                                @lang('Sell '){{$item->cryptoCurrency->code}}</a>
                        </td>
                    </tr>
                @empty
                    <tr>
        ...
        <nav aria-label="Page navigation example">
            <ul class="pagination justify-content-center">
                {{ $sellLists->appends($_GET)->links($theme.'partials.pagination') }}
            </ul>
        </nav>
    </div>
@endsection
